...
function checkdaftar() {
	if (!isset($_SESSION['id_user'])) {
		setAlert("Access Denied!", "daftar First!", "error");
		header('Location: daftar.php');
	} 
}
function checkdaftarAtdaftar() {
	if (isset($_SESSION['id_user'])) {
		setAlert("You has been logged!", "Welcome!", "success");
		header('Location: login.php');
	}
...
function addPemasukan($data) {
	global $conn;
	$id_user = htmlspecialchars($_SESSION['id_user']);
	$jumlah_pengeluaran = htmlspecialchars($data['jumlah_pemasukan']);
	$keterangan = htmlspecialchars($data['keterangan']);
